from TwoLocusGPR import GPR, MSD_functions
import numpy as np
...
def test_predictions():
    ...
    regressor = GPR(times, MSD_functions.Rouse_MSD, ndims)

    theta = np.array([Gamma, J] * ndims + noise)
    true_dat, noisy_dat = regressor.sample_prior(
        theta, ntracks, nanfrac=nanfrac, seed=12
    )
